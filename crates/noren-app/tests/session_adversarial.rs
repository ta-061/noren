...
// ═════════════════════════════════════════════════════════════════════════════
// Domain registry — reported defects (ignored reproducers; FAIL when run).
// ═════════════════════════════════════════════════════════════════════════════

#[test]
#[ignore = "reproduces ADV-S1: observe permits non-monotonic status regression"]
fn adv_s1_observe_can_regress_and_resurrect_status() {
    // The module doc states observe "advances a session past Created", implying
    // forward-only transitions. The implementation has no direction guard, so
    // a status can move *backwards*. This both hides a running session
    // (Running -> Created) and resurrects a dead one (Exited -> Running).
    let mut reg = dom::SessionRegistry::new();
    let id = reg.create(dom::SessionKind::Local, None);
    reg.observe(id, dom::SessionStatus::Running).expect("fwd");
    assert_eq!(reg.get(id).unwrap().status(), dom::SessionStatus::Running);

    // Regression 1: Running -> Created. A successful observation should never
    // un-observe a session. Expected: rejected or no-op. Actual: it succeeds.
    reg.observe(id, dom::SessionStatus::Created)
        .expect("regressed");
    assert_eq!(
        reg.get(id).unwrap().status(),
        dom::SessionStatus::Running,
        "observe(Created) regressed a Running session; status must be monotonic"
    );

    // Regression 2: Exited -> Running. A dead session is resurrected as live,
    // which is the most alarming form for a lifecycle model.
    reg.observe(id, dom::SessionStatus::Exited).expect("fwd2");
    reg.observe(id, dom::SessionStatus::Running)
        .expect("resurrected");
    assert_eq!(
        reg.get(id).unwrap().status(),
        dom::SessionStatus::Exited,
        "observe(Running) resurrected an Exited session; terminal status must be sticky"
    );
}
...
// ═════════════════════════════════════════════════════════════════════════════
// Supervisor — reported defects (ignored reproducers; FAIL when run).
// ═════════════════════════════════════════════════════════════════════════════

#[test]
#[ignore = "reproduces ADV-S2: supervisor retains dead-session records without bound"]
fn adv_s2_dead_records_accumulate_without_bound() {
    // Boundedness probe (the brief's explicit warning). The domain is bounded
    // because Close removes entries (invariant #4). The supervisor has no
    // equivalent: poll/terminate mark a session terminal but RETAIN its record;
    // only forget() retires it, and nothing requires or enforces forget. So the
    // realistic app pattern — spawn sessions, let them die, open more — grows
    // the session list without limit. This is the same class of bug as the
    // cell-growth defect a prior sweep found.
    let children: Vec<_> = (0..500).map(|_| MockChild::exited()).collect();
    let mut s = supervisor_with(children);
    for _ in 0..500 {
        let _id = s.spawn().expect("spawn");
        let _ = s.poll(); // reaps immediately -> Exited; record RETAINED
        // (no forget — reap-and-move-on is the natural pattern)
    }
    assert_eq!(s.running_count(), 0, "no session should still be running");
    // Boundedness: dead records must not accumulate. The domain guarantees this;
    // the supervisor does not.
    assert!(
        s.len() <= 1,
        "supervisor retained {} dead records after 500 spawn/crash cycles; \
         list grows without bound (no auto-retirement, no cap)",
        s.len()
    );
}

#[test]
#[ignore = "reproduces ADV-S3: terminate on unknown id fabricates Failed(PollFailed)"]
fn adv_s3_terminate_unknown_id_fabricates_status() {
    // terminate returns SessionStatus (not Result), so there is no Unknown
    // channel. For an id it no longer tracks, it returns Failed(PollFailed): a
    // control-plane status describing a poll failure that never happened on a
    // session that does not exist. A caller cannot distinguish "I terminated a
    // dead session" from "that id never existed", and a bogus status leaks out.
    let mut s = supervisor_with(vec![MockChild::running()]);
    let id = s.spawn().expect("spawn");
    s.terminate_now(id); // -> Exited, terminal
    s.forget(id).expect("forget terminal");
    assert_eq!(s.status(id), None); // genuinely unknown now

    let returned = s.terminate_now(id);
    assert_ne!(
        returned,
        SessionStatus::Failed {
            reason: SessionFailure::PollFailed
        },
        "terminate on an unknown id fabricated Failed(PollFailed) instead of \
         signaling that the id is unknown"
    );
}
