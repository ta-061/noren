...
The original research reviewed sanitized `--help` captures in PR #12. Those
machine-specific artifacts are no longer shipped in the product repository;
the table preserves the versions and capabilities inspected at that historical
checkpoint. Any future adapter must capture and verify its own current evidence
rather than treating this table as an enablement gate.

| Agent | Local executable evidence | Version inspected | Relevant locally advertised capabilities |
| --- | --- | --- | --- |
| Codex | Sanitized CLI and exact `exec resume --help` evidence reviewed in PR #12 | `codex-cli 0.146.0` | hook-trust bypass flag (negative evidence only); plugins; `exec --json`; output schema/file; top-level resume and exact `exec resume` argv/flags contract; experimental app server |
| Claude Code | Sanitized CLI evidence reviewed in PR #12 | `2.1.220` | hooks/plugins; `--output-format json/stream-json`; `--include-hook-events`; JSON schema; session/resume |
| OpenCode | Corrected selected/shadowed executable evidence reviewed in PR #12 | Selected `1.18.11`; shadowed npm-global `1.14.31` also inventoried | plugins; `run --format json`; serve/attach; ACP; session continuation |

Authentication checks in those historical captures were redacted. This
document does not repeat credential locations or account/provider details.
...
executable identity. The corrected capture reviewed in PR #12 recorded the
selected user-local file and digest, both shadowed npm artifacts and digests,
and the package-manager check. Its absolute-path `--version`, `--help`, and
`run --help` revalidation used the intended user-local file and supports the
historical capabilities summarized here without querying credentials. The
original pre-PR #10 evidence incorrectly paired
...
For Codex, the exact `exec resume --help` capture reviewed in PR #12 recorded
the `0.146.0` argv/flags contract. Its provenance was valid for that research
checkpoint because the accompanying inventory showed that the isolated-
`CODEX_HOME` `codex-lab` wrapper resolved the same
`/opt/homebrew/bin/codex` executable and `codex-cli 0.146.0` version as the
direct Codex evidence. The optional Codex
...
enabling each resume adapter. The historical capture documents the research
result but, because it is no longer shipped, does not by itself satisfy a future
enablement gate; any identity or version change also invalidates it. CORE-06
verifies valid correlated-session resume. Valid-looking
