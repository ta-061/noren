...
| `Include` | Includes one or more files. In a user config, every relative path is based at the user configuration directory, including a directive in an included file; it is not based at that file's parent. Paths may use globbing, tokens, environment variables, or `~`; wildcard expansion is lexical. | A partial parser can miss nested or ordered policy, including security settings, or silently select a same-name file from the wrong directory. | SSH-02 |
...
The option definitions, first-value precedence, and user-config base for
relative `Include` paths are in
...
| SSH-02 | Include/Match/token/precedence | Generate nested root-relative includes with a root-level target and a same-name decoy beside the including file, lexical globs, `~`, first-value conflicts, ordered `Host`/`Match` blocks, and harmless `Match exec` marker predicates. Compare effective configuration and expected marker execution with the isolated `ssh -G` oracle; a candidate that ignores `Match` fails unless it explicitly marks the destination unsupported. | M-ARM, L-X64 |
