...
| Fact | Value |
| --- | --- |
| Artifact | `dist/noren-0.1.0-preview-<sha>-aarch64-apple-darwin.tar.gz` (two files inside: the `noren-app` binary, ~6.9 MB, and `RELEASE-NOTES.md`) |
| Release notes | Generated by `build.sh` at the exact commit being packaged and packed into the artifact; no committed notes file exists (a committed copy pins an ancestor and is stale at every merged tip unless the tip is itself a notes commit — the #187 deadlock) |
...
| Command | Result |
| --- | --- |
| `scripts/release/build.sh` | Artifact + `RELEASE-NOTES.md` + `SHA256SUMS` + `BUILD-PROVENANCE.txt` produced; `shasum -c` OK; the build ran on a clean, freshly merged `main` with no notes re-pin step (#187 deadlock gone) |
| `scripts/release/build.sh` (second run, same commit) | Identical `ARTIFACT_SHA256` |
| `scripts/release/build.sh --check-repro` | `REPRODUCIBLE`: working tree + two clean worktrees, identical digests |
| Stale-notes handling (throwaway branch, #187) | A committed notes file pinning the non-ancestor `d4ec52a` (the deadlock trigger) no longer blocks the build — it is ignored, and the artifact's `RELEASE-NOTES.md` pins the artifact's own commit; a sabotaged generator emitting a wrong pin is refused (`the notes would describe a history that did not ship`, exit 1) |
| Dirty-source handling | A modified `crates/noren-app/src/main.rs` still refuses: `working tree is dirty … the artifact name embeds HEAD and must not lie` (exit 1) |
