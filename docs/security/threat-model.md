...
| TM-08 | Logs, panic reports, snapshots, CI artifacts, or review comments disclose terminal contents, environment values, secrets, or private paths. | Log only component/operation/status and coarse sizes; redact cwd to an opt-in diagnostic; do not persist terminal frames or input outside local test fixtures. File-derived errors (config, session state, ssh config) echo key names and positions only, never file values — the sole allowlist is `[keys]` chord text, which is keybinding grammar bounded and clipped to 120 characters (issue #150); a classifier test fails the build on any new echoing variant. | Secret-pattern check plus a test logger that rejects known sentinel terminal/input/environment values. |
...
| TM-10 | A configured agent command (`[[agents]]`) becomes a shell-injection or binary-substitution vector. | The launch is an argv vector, never `sh -c`: the command is `argv[0]` and each `args` element is exactly one argv word, so `;`, `$(...)`, and backticks in any configured value are literal data (no shell interprets them). The command must be an absolute path with a leading `/` — `PATH` lookup is never performed, so a writable `PATH` entry cannot substitute a different binary (the same reasoning that fixes the SSH client at `/usr/bin/ssh`). Validation is enforced twice (configuration load and the PTY launch policy) with typed, content-free errors; the command and args never reach a Debug surface, an error `Display`, or `sessions.toml` (only the display name persists); a missing or non-executable command is a visible per-row and status-row failure, never a hang or a silent no-op. | argv-pinning tests (no shell word is added, split, or re-quoted), typed-refusal tests for empty/relative commands, sentinel probes against Debug/Display/state-file surfaces under the file-echo contract, and a real-child test that reads `/bin/echo`'s own output back through the PTY. |
