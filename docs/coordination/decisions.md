...
| D-0010 | 2026-08-05 | Coordinate development as an agent fleet: GLM owns the terminal core, Qwen the application layer, Kimi adversarial robustness, codex-lab integration; the Claude Code session coordinates and does not implement what a lane can. | The previous single-integrator model exhausted `codex-tatu` to ~10% weekly and forced a coordinator handoff. Lanes are scoped so no two engines review the same code, and dispatch is quota-gated so an exhausted engine hands off instead of failing mid-task. | Active; see [fleet](fleet.md) |
