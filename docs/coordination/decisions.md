...
| D-0008 | 2026-08-03 | Require the non-overlapping Issue #8 reviews before closure: `codex-lab` tests testability/gates and `Claude Code` tests security/maintainability of the risk register; Codex integrates and decides. | Issue #8's required verification and the design council protocol both require addressed independent reviews before the artifact becomes Round 1 shared input. Assignments are recorded in the PR and [risk register](../roadmap/risk-register.md); no review verdict is claimed yet. | Active |
