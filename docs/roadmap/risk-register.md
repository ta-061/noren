...
| R-SSH-01 | SSH | Noren silently reinterprets OpenSSH config semantics (Host/Match/Include/ProxyJump precedence, pinned `russh-config` limits) or weakens host-key verification, enabling wrong security settings or MITM | SR reinterpretation table + pinned `russh-config` limits + SSH-05; LC §7–§8; OQ host-key UI question | L4 | I5 | Deferred SSH design owner; `Claude Code` review; `codex-lab` differential fixtures | Semantics-dependent destinations fall back to ordinary OpenSSH marked unsupported, never approximated; differential fixtures vs pinned `ssh -G` (kept redacted); strict known-hosts preserved; never auto-accept prompts; atomic persisted accepted keys; host-key mismatch never auto-retries | SSH transport/config resolver selection; embedded-transport PoC; first-use UI design | Design: OQ OpenSSH subprocess vs library; first-use host-key confirmation. Release: SR SSH-01–SSH-05; CP `parity_ssh_config_fixture` |
...
| R-PORT-01 | Portability | macOS/Wayland/X11 input/windowing/IME/a11y divergence with a single window/event candidate; no Rust toolchain installed, so buildability and MSRV are unproven and every executable gate in this register is blocked | LC §3B gap + §6 IME coupling; TL unknowns; ZJ `Z-MAC`/`Z-WAYLAND`/`Z-X11`; ST blocked section; AC/agent-inventory (cargo/rustc/rustup absent) | L4 | I4 | Design council; human owner (ta-061) approves toolchain; `codex-lab` measurements | Install and pin a toolchain before the first compile experiment and record versions/targets; identical lifecycle/input/IME traces on AppKit, Wayland, X11 before any adoption ADR; second window candidate or recorded market-gap justification; X11 support is a product decision | M1 window/event selection; first experiment requiring compilation | Design: OQ stack question + MSRV/toolchain question. Release: LC §3B `winit` trace PoC; ZJ `zellij_japanese_ime` on `Z-MAC`/`Z-WAYLAND`/`Z-X11`; toolchain absence itself is a blocking dependency with no release gate |
...
| Preview MSRV and reproducible Rust toolchain installation/pinning | Explicit deferral until the first Rust experiment | Blocking dependency tracked in R-PORT-01; design council decides with recorded toolchain evidence |
